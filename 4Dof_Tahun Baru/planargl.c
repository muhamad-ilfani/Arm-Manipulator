  ...
  if (gerak == 1) {
    ...
    while (k <= N*PERIOD) {
      ...
      forward_kinematics();
  	  ...
  	  gerak1=0;
  	  printf("%.4f %.4f %.4f\n", q1*RTD-90, q2*RTD,q3*RTD);
      if (counter%((N/divider)) == 0) {
        //printf("%.4f %.4f %.4f %.4f %.4f %.4f %.4f\n",y,-x,z, q1*RTD-90, q2*RTD,q3*RTD,q4*RTD);
...
//        if (sudut3 < 0) sudut3 += 360;
        ...
        check=kirimsudut1_0^kirimsudut1_a^kirimsudut1_b^kirimsudut2_0^kirimsudut2_a^kirimsudut2_b^kirimsudut3_0^kirimsudut3_a^kirimsudut3_b^sudut4;
...
void keyboard(unsigned char key, int i, int j)
{
     ...
     case '2' : z_final = z_final+0.05; gerak=1; break;
     case '3' : z_final = z_final-0.05; gerak=1; break;
	 case '4' : y_final = y_final+0.02; gerak=1; break;
	 case '5' : y_final = y_final-0.02; gerak=1; break;
	 case '6' : x_final =x_final+0.01; gerak = 1; break;
	 case '7' : x_final =x_final-0.01; gerak = 1; break;
